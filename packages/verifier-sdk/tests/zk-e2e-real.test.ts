...
describe("Real ZK E2E Tests", () => {
  ...
  it("should verify real ZK proof with valid age over threshold", async () => {
    ...
    const result = verifier.verifyProofResponse(proofResponse);
    
    // Verification should succeed with valid real proof
    expect(result.verified).toBe(true);
    expect(result.claims.some(c => c.type === "AGE_OVER" && c.value === true)).toBe(true);
  });
  ...
  it("should verify multiple ZK proofs in sequence without interference", async () => {
    ...
    const result1 = verifier.verifyProofResponse(resp1);
    const result2 = verifier.verifyProofResponse(resp2);
