...
export async function captureDocumentImage(
  ...
  return new Promise((resolve, reject) => {
    canvas.toBlob((blob) => {
      if (!blob) reject(new Error("CAPTURE_FAILED"));
      else resolve(blob);
    }, "image/jpeg", 0.92);
  });
}
...
function parseFieldsSimple(text: string): OCRResult {
  ...
  return {
    givenName: nameMatch?.[1]?.trim().split(/\s+/)[1] ?? "",
    familyName: nameMatch?.[1]?.trim().split(/\s+/)[0] ?? "",
...
export async function performOCR(imageBlob: Blob): Promise<OCRResult> {
  if (imageBlob.size > 5 * 1024 * 1024) throw new Error("IMAGE_TOO_LARGE");
  if (!imageBlob.type.startsWith("image/")) throw new Error("INVALID_IMAGE_TYPE");
  ...
  try {
    ...
    if (!recognizer) {
      console.warn("[OCR] No recognizer available; user confirmation required");
      return {
        givenName: "",
        familyName: "",
        dateOfBirth: "",
    ...
    const result = recognizer.recognize(canvas);
    return parseFieldsSimple(result.text ?? "");
  } catch (err) {
...
export function normalizeAttribute(value: string, type: string): string {
  if (!value) return "";
  if (type === "givenName" || type === "familyName") return value.trim().toUpperCase();
